...
fn release_tag_override_forces_stable_version_output() {
    ...
    assert_eq!(metadata.long_version, "0.2.6 (Rust port of flowmark-py 0.7.0; base v0.2.6)");
}

#[test]
fn no_git_metadata_defaults_to_stable_version_output() {
    let metadata = compute_version_metadata("0.2.6", "0.7.0", None, None, None, None);
...
fn commits_ahead_produces_dev_suffix() {
    ...
    assert_eq!(
        ...
        "0.2.6-dev.4+gabcdef0 (Rust port of flowmark-py 0.7.0; base v0.2.5)"
    );
}
