...
fn test_heading_spacing_basic() {
    // Our comrak-based renderer outputs heading\n\n then paragraph\n, so there's
    // no additional blank line before subsequent headings (consistent with Python output).
...
fn test_heading_spacing_before_list() {
    ...
    assert_eq!(fm_loose(input), expected);
}
