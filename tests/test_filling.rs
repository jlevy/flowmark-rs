//! Integration tests for `fill_markdown`, ported from Python `test_filling.py`.
...
"#
    ...
    let expected_doc = r#"# This is a header
...
Seven. Eight. Nine. Ten.
A [link](https://example.com). Some *emphasis* and **strong emphasis** and `code`. And a
...
"#;
    ...
    assert_eq!(DEFAULT_WRAP_WIDTH, 88);
...
fn test_multi_paragraph_list_items() {
    let input = r#"- **`make_parent_dirs(path: str | Path, mode: int = 0o777) -> Path`**
...
"#;
    ...
    let expected = r#"- **`make_parent_dirs(path: str | Path, mode: int = 0o777) -> Path`**
  Ensures that the parent directories for a file exist, creating them if necessary.
