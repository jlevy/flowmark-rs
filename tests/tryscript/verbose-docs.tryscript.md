...
Tests for --skill, --install-skill (including cross-agent --surfaces), and --docs output.
Note: --verbose is Rust-only (not in Python). The --docs body is the Rust port's own
README, so its headings differ from the Python build; the skill (--skill) and the
installed skill artifacts are byte-identical across both implementations.
...
```console
$ mkdir v14 && cd v14 && flowmark --install-skill --surfaces=agents-md >/dev/null && grep -Fx "<!-- BEGIN FLOWMARK INTEGRATION format=f02 surface=agents-md -->" AGENTS.md && grep -Fx "<!-- END FLOWMARK INTEGRATION -->" AGENTS.md
