...
- [x] Fix `fmr-0pxh` in the bridge, at `repair_synthetic_list_looseness`. Output matches
  Python byte for byte, and upstream pins the exact bytes as the shared case
...
$ printf -- '- Verse text follows\n  | first "raw"... line\n' |
    flowmark --width 0 --smartquotes --ellipses -
- Verse text follows
  | first “raw”… line     # protection dropped; Python keeps the authored bytes
```

Recognition has to stay identical to the reference, so neither rule can stand.
